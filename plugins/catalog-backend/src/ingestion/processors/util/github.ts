...
export async function getOrganizationUsers(
      ...
      query users($org: String!, $cursor: String) {
        organization(login: $org) {
          membersWithRole(first: 100, after: $cursor) {
            ...
            node { avatarUrl, bio, email, login, name }
          }
        }
      }`;
  ...
  for (let i = 0; i < 100 /* just for sanity */; ++i) {
    ...
    for (const user of connection.node) {
      users.push({
...
export async function getOrganizationTeams(
      ...
      organization(login: $org) {
        teams(first: 100, after: $cursor) {
          ...
          node {
            ...
            combinedSlug
            parentTeam { slug }
            members(first: 100, membership: TeamMembershipType.IMMEDIATE) {
              pageInfo { hasNextPage, endCursor }
              node { login }
            }
          }
  ...
  for (let i = 0; i < 100 /* just for sanity */; ++i) {
    ...
    for (const team of connection.node) {
      groups.push({
        apiVersion: 'backstage.io/v1alpha1',
        kind: 'Group',
        metadata: {
      ...
      if (!team.members.pageInfo.hasNextPage) {
        ...
        for (const user of team.members.node) {
...
export async function getTeamMembers(
  ...
  const query = `
    query members($org: String!, $teamSlug: String!, $cursor: String) {
      organization(login: $org) {
        team(slug: $teamSlug) {
          members(first: 100, after: $cursor, membership: TeamMembershipType.IMMEDIATE) {
            pageInfo { hasNextPage, endCursor }
            node { login }
          }
        }
